...
class ProgressManager {
	...
	class func checkAndSetAchievementReached(view: UIViewController, type: String) {
		...
		else if type == Constants.Achievements.SESSIONS_TYPE {
			for achievement in Constants.Achievements.ACHIEVEMENTS_SESSIONS {
				let value = Int(achievement[0].components(separatedBy: "s")[0])
				if Utils.int(key: Constants.Defaults.APP_DATA_TOTAL_SESSIONS) >= value! {
					checkAchivementReached(view: view, achievement: achievement)
				}
			}
		}
		else if type == Constants.Achievements.TIME_MEDITATING_TYPE {
			for achievement in Constants.Achievements.ACHIEVEMENTS_TIME_MEDITATING {
				let value = Int(achievement[0].components(separatedBy: "t")[0])
				if Utils.int(key: Constants.Defaults.APP_DATA_TOTAL_TIME_MEDITATING) >= value! {
					checkAchivementReached(view: view, achievement: achievement)
				}
			}
		}
