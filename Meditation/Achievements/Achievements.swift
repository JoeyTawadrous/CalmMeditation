...
class Achievements: UIViewController, UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout {
	...
	/////////////////////////////////////////// */
	func initPointsView() {
		circularPointsView = createProgressCircle(x: (UIScreen.main.bounds.width / 2) - 50, color: Constants.Colors.PRIMARY_PURPLE)
	...
	func createProgressLabel(center: CGPoint) -> UILabel {
		...
		label.center = center
		label.textColor = UIColor(hex: Constants.Colors.PRIMARY_TEXT_GRAY)
	...
	func createProgressCircle(x: CGFloat, color: String) -> KYCircularProgress {
		let progressView = KYCircularProgress(frame: CGRect(x: x, y: 5, width: 100, height: 100), showGuide: true)
	...
	func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
		...
		if Utils.bool(key: "FASTLANE_SNAPSHOT") {
			...
			Utils.createHexagonImageView(imageView: cell.containerImageView, lineWidth: 5, lineColor: UIColor(hex: achievement[1]))
		}
		else {
			if !ProgressManager.isAchievementReached(name: achievement[0]) {
				...
				cell.iconImageView.image = image
				cell.titleLabel.textColor = UIColor(hex: Constants.Colors.PRIMARY_TEXT_GRAY)
				Utils.createHexagonImageView(imageView: cell.containerImageView, lineWidth: 5, lineColor: UIColor(hex: achievement[1]))
			}
		}
